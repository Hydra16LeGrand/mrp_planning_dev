...
class MrpPlanning(models.Model):
    ...
    section_id = fields.Many2one("mrp.section", string=_("Section"), required=True, tracking=5)

    team_ids = fields.Many2many("mrp.team", string=_("Teams"), tracking=True, copy=True, required=True)
    ...
    def _compute_detailed_pl_done_state(self):
        for rec in self:
            ...
            if rec.detailed_pl_ids:
                mrp_detail_states = [detail.state for detail in rec.detailed_pl_ids if not detail.display_type]
        ...
        try:
            for pline in self.planning_line_ids:
                val = {
                    ...
                    'team_id': pline.team_id,
                    'mrp_days_ids': pline.mrp_days_ids,
                    'planning_id': self.id,
            ...
            if rec.state == 'draft':
                ...
                for detailed_planning_line in rec.detailed_pl_ids:
                    if not detailed_planning_line.display_type:
                        ...
                        team_id = detailed_planning_line.team_id
                        grouped_lines[section][packaging_line].append((team_id, detailed_planning_line))

                result_dict = {}
                for section, packaging_lines in grouped_lines.items():
                    section_dict = {}
                    for packaging_line, teams in packaging_lines.items():
                        merged_teams = tuple(sorted(set([team for team, _ in teams])))
                        section_dict[(packaging_line, merged_teams)] = [line for _, line in teams]
    ...
    def copy(self, default=None):
        ...
        if self.planning_line_ids:
            new_planning_line_ids = [(0, 0, {
                ...
                'mrp_days_ids': [(6, 0, line.mrp_days_ids.ids)],
    ...
    def write(self, vals):
        for rec in self:
            ...
            old_team = [team.name for team in rec.team_ids]
            ...
            if 'team_ids' in vals:
                new_team_id = [team for val in vals['team_ids'] for team in val[2]]
                new_team_id_name = []
                for team in new_team_id:
                    team_name = self.env['mrp.team'].browse(team)
                    new_team_id_name.append(team_name.name)

                formatted_old_team_name = ', '.join(map(str, old_team))
                formatted_new_team_id_name = ', '.join(map(str, new_team_id_name))
                message_to_day = f"<p><b> {formatted_old_team_name} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{formatted_new_team_id_name}</span></b><em> (Teams)</em></p>"
    ...
    def _get_new_section_name(self, vals):
        if isinstance(vals, dict):
            new_sect_id = vals.get('section_id', False)
            if new_sect_id:
        ...
        for pl in update_pl:
            for planning in old_planning_line:
                if pl['id'] == planning['id']:
                    ...
                    if 'team_id' in value:
                        new_team = self.env['mrp.team'].browse(value['team_id'])
                        msg += f"<li><p><b>Team {planning['team_id']['name']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{new_team.name}</span></b></p></li>"

                    if 'begin_date' in value:
                        msg += f"<li><p><b>{planning['begin_date']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{value['begin_date']}</span></b> <i>(Begin Date)</i></p></li>"

                    if 'end_date' in value:
                        msg += f"<li><p><b>{planning['end_date']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{value['end_date']}</span></b> <i>(End Date)</i></p></li>"
                ...
                for detail in old_detail_planning_line:
                    if dl['id'] == detail['id']:
                        if dl['id'] in section_dls:
                            ...
                            if 'team_id' in value:
                                msg += f"<li><p><b>{detail['team_id']['name']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>Team {value['team_id']['name']}</span></b> <i>(Team)<i></p></li>"
...
class MrpPlanninLine(models.Model):
        ...
        for rec in self:
            if rec.begin_date and rec.end_date:
                ...
                while current_date <= rec.end_date:
                    ...
                    mrp_days_ids.append((4, day_record.id, False))
                    current_date += timedelta(days=1)
                rec.mrp_days_ids = mrp_days_ids
            else:
                rec.mrp_days_ids = False
    ...
    mrp_days_ids = fields.Many2many('mrp.planning.days', string='Mrp Days', compute="_compute_mrp_days")
...
class MrpDetailPlanningLine(models.Model):
    ...
    section_id = fields.Many2one("mrp.section", related="planning_id.section_id")
    planning_id = fields.Many2one("mrp.planning", tracking=True)
    packaging_line_domain = fields.Many2many("mrp.packaging.line", compute="_compute_packaging_line_domain")
    ...
    qty_done = fields.Integer(_("Quantity done"), tracking=True)
    team_id = fields.Many2one('mrp.team', string=_("Team"))
