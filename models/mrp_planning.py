...
class MrpPlanning(models.Model):
    ...
    def create_lines_or_sections(self, element, dt):
        name = '       ' + element['packaging_line_id'].name if dt == 'line_note' else element['section_id'].name
        self.env['mrp.detail.planning.line'].create([{
            'display_type': dt,
            'name': name,
            'product_id': element['product_id'].id,
                    ...
                    for element in liste_meme_section_id:
                        ...
                        if liste_meme_packaging_line_id:
                            for elm in liste_meme_packaging_line_id:
                                ...
                                for value in elm:
                                    self.create_detail_planning_line(value)

                        if liste_autres_dico:
                            for value in liste_autres_dico:
                                self.create_lines_or_sections(value, 'line_note')
                                self.create_detail_planning_line(value)
    ...
    #
    #         # Appeler la méthode de base write
    #         res = super(MrpPlanning, rec).write(vals)
    #
    ...
    #         self._process_planning_line_changes(vals, old_planning_line, mrp_planning)
    #         self._process_detail_planning_line_changes(vals, old_detail_planning_line, mrp_planning)
    #
    #         return res
    #
    ...
    #
    # def _process_week_of_changes(self, vals, old_day_name, mrp_planning):
    #     if 'week_of' in vals:
    #         new_week_of_name = self._get_new_week_of_names(vals)
    #         formatted_old_week_of_name = ', '.join(map(str, old_day_name))
    #         formatted_new_week_of_name = ', '.join(map(str, new_week_of_name))
    #         message_to_day = f"<p><b> {formatted_old_week_of_name} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{formatted_new_week_of_name}</span></b><em> (Week of)</em></p>"
    #         mrp_planning.message_post(body=message_to_day)
    #
    # def _get_new_week_of_names(self, vals):
    #     new_day_id = [day for val in vals['week_of'] for day in val[2]]
    #     new_week_of_name = [self.env['mrp.planning.days'].browse(day).name for day in new_day_id]
    #     return new_week_of_name
    #
    # def _process_planning_line_changes(self, vals, old_planning_line, mrp_planning):
    #     added_planning_lines, deleted_planning_lines, updated_planning_lines = self._extract_planning_changes(vals, 'planning_line_ids')
    #
    #     for update_pl in updated_planning_lines:
    #         planning = self._find_matching(old_planning_line, update_pl['id'])
    #         if planning:
    #             self._process_planning_line_update(planning, update_pl['value'], mrp_planning)
    #
    #     self._process_deleted_planning_lines(deleted_planning_lines, old_planning_line, mrp_planning)
    #     self._process_added_planning_lines(added_planning_lines, mrp_planning)
    #
    # def _extract_planning_changes(self, vals, field):
    #     lines = vals.get(field, [])
    #     added_lines = [val[2] for val in lines if val[0] == 0]
    #     deleted_lines = [val[1] for val in lines if val[0] == 2]
    #     updated_lines = [{'id': val[1], 'value': val[2]} for val in lines if val[0] == 1]
    #     return added_lines, deleted_lines, updated_lines
    #
    # def _find_matching(self, old_line, line_id):
    #     return next((line for line in old_line if line['id'] == line_id), None)
    #
    # def _process_planning_line_update(self, planning, update_value, mrp_planning):
    #     message = self._build_planning_update_message(planning, update_value)
    #     if len(message) > 10:
    #         mrp_planning.message_post(body=message)
    #
    # def _build_planning_update_message(self, planning, update_value):
    #     message = f"<p><b><em> (Planning lines)</em> Packaging Line {planning['packaging_line_id']['name']}, Section {planning['section_id']['name']}:</b></p><ul>"
    #
    #     for field in ['product_id', 'packaging_line_id', 'qty', 'section_id', 'mrp_days', 'uom_id']:
    #         if field in update_value:
    #             old_value = planning.get(field, '')
    #             new_value = update_value[field]
    #             if field == 'mrp_days':
    #                 old_value = ', '.join([day['name'] for day in old_value])
    #                 new_value = ', '.join([self.env['mrp.planning.days'].browse(day[2]).name for day in new_value])
    #             elif field == 'product_id':
    #                 old_value = planning['product_id']['name']
    #                 new_value = self.env['product.product'].browse(new_value).name
    #             elif field == 'packaging_line_id':
    #                 old_value = planning['packaging_line_id']['name']
    #                 new_value = self.env['mrp.packaging.line'].browse(new_value).name
    #             elif field == 'section_id':
    #                 old_value = planning['section_id']['name']
    #                 new_value = self.env['mrp.section'].browse(new_value).name
    #             message += f"<li><p><b>{field.capitalize()} {old_value} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{new_value}</span></b></p></li>"
    #     if message == f"<p><b><em> (Planning lines)</em> Packaging Line {planning['packaging_line_id']['name']}, Section {planning['section_id']['name']}:</b></p><ul>":
    #         message = ""
    #     else:
    #         message += "</ul>"
    #     return message
    #
    # def _process_deleted_planning_lines(self, deleted_planning_lines, old_planning_line, mrp_planning):
    #     message = self._build_deleted_planning_message(deleted_planning_lines, old_planning_line)
    #     if len(message) > 10:
    #         mrp_planning.message_post(body=message)
    #
    # def _build_deleted_planning_message(self, deleted_planning_lines, old_planning_line):
    #     message = "<p><em>Planning lines removed are :</em></p><ul>"
    #     for pl in deleted_planning_lines:
    #         planning = self._find_matching(old_planning_line, pl)
    #         if planning:
    #             message += f"<li><p><b>{planning['product_id']['name']}, section {planning['section_id']['name']}, packaging line {planning['packaging_line_id']['name']}</b></p></li>"
    #
    #     if message == "<p><em>Planning lines removed are :</em></p><ul>":
    #         message = ""
    #     else:
    #         message += "</ul>"
    #     return message
    #
    # def _process_added_planning_lines(self, added_planning_lines, mrp_planning):
    #     message = self._get_pl_message(added_planning_lines, "Planning lines added are")
    #     if message:
    #         mrp_planning.message_post(body=message)
    #
    # def _process_detail_planning_line_changes(self, vals, old_detail_planning_line, mrp_planning):
    #     new_dl_id, deleted_dl_ids, updated_dl = self._extract_planning_changes(vals, 'detailed_pl_ids')
    #
    #     section_dl_ids, line_dl_ids = self._get_section_and_line_ids(old_detail_planning_line)
    #     update_dl_ids = [dl['id'] for dl in updated_dl]
    #
    #     self._process_updated_detail_planning_lines(section_dl_ids, line_dl_ids, update_dl_ids, old_detail_planning_line, mrp_planning, updated_dl)
    #     self._process_deleted_detail_planning_lines(deleted_dl_ids, old_detail_planning_line, section_dl_ids,
    #                                                 line_dl_ids, mrp_planning)
    #
    # def _get_section_and_line_ids(self, old_detail_planning_line):
    #     section_dl = [dl['id'] for dl in old_detail_planning_line if dl['display_type'] == 'line_section']
    #     line_dl = [dl['id'] for dl in old_detail_planning_line if dl['display_type'] == 'line_note']
    #     section_dls = sorted(section_dl)
    #     line_dls = sorted(line_dl)
    #     return section_dls, line_dls
    #
    # def _process_updated_detail_planning_lines(self, section_dls, line_dls, update_dl_ids, old_detail_planning_line, mrp_planning, updated_dl):
    #     for dl in update_dl_ids:
    #         for update in updated_dl:
    #             if dl == update['id']:
    #                 detail = next((detail for detail in old_detail_planning_line if detail['id'] == dl), None)
    #                 if detail:
    #                     if detail['display_type'] == 'line_section':
    #                         self.print_message(mrp_planning, 'mrp.detail.planning.line', dl, detail, "",
    #                                                                            " (Detailed planning lines Large Sections)")
    #                     elif detail['display_type'] == 'line_note':
    #                         self.print_message(mrp_planning, 'mrp.detail.planning.line', dl, detail, "",
    #                                                                            " (Detailed planning lines Large Sections)")
    #                     else:
    #                         # Récupérer la ligne et la section de l'élément modifié
    #                         sect_dl = max([sect_id for sect_id in section_dls if sect_id < dl], default=None)
    #                         line_dl = max([line_id for line_id in line_dls if line_id < dl], default=None)
    #                         large_section = self.env['mrp.detail.planning.line'].browse(sect_dl)
    #                         large_line = self.env['mrp.detail.planning.line'].browse(line_dl)
    #                         self._process_large_section_and_line_update(mrp_planning, dl, detail, large_section, large_line, update)
    #
    # def _process_large_section_and_line_update(self, mrp_planning, dl, detail, large_section, large_line, update):
    #     message = f"<p><b><em> (Detailed planning lines)</em> Large Section {large_section.name}, Large Line {large_line.name}, Date {detail['date'].strftime('%d/%m/%Y')}:</b></p><ul>"
    #
    #     for field in ['date', 'product_id', 'packaging_line_id', 'section_id', 'qty']:
    #         if field in update:
    ...
    #                 new_value = ', '.join([self.env['mrp.planning.days'].browse(day[2]).name for day in new_value])
    #             elif field == 'product_id':
    #                 old_value = detail['product_id']['name']
    #                 new_value = self.env['product.product'].browse(new_value).name
    #             elif field == 'packaging_line_id':
    #                 old_value = detail['packaging_line_id']['name']
    #                 new_value = self.env['mrp.packaging.line'].browse(new_value).name
    #             elif field == 'section_id':
    #                 old_value = detail['section_id']['name']
    #                 new_value = self.env['mrp.section'].browse(new_value).name
    #             elif field == 'qty':
    #                 old_value = detail['qty']
    #                 new_value = dl['qty']
    #             message += f"<li><p><b>{field.capitalize()} {old_value} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{new_value}</span></b></p></li>"
    #     if message != f"<p><b><em> (Detailed planning lines)</em> Large Section {large_section.name}, Large Line {large_line.name}, Date {detail['date'].strftime('%d/%m/%Y')}:</b></p><ul>":
    #         message += "</ul>"
    #         mrp_planning.message_post(body=message)
    #
    #
    # def _process_large_section_and_line_update_value(self, value, detail):
    #     msg = ""
    #     if 'date' in value:
    #         msg += f"<li><p><b>Date {detail['date']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{value['date']}</span></b></p></li>"
    #
    #     if 'product_id' in value:
    #         new_prod = self.env['product.product'].browse(value['product_id'])
    #         msg += f"<li><p><b>{detail['product_id']['name']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{new_prod.name}</span></b></p></li>"
    ...
    #     return msg
    #
    # def _process_deleted_detail_planning_lines(self, deleted_dl_ids, old_detail_planning_line, section_dl_ids,
    #                                            line_dl_ids, mrp_planning):
    #     message = "<p><em>Detailed planning lines removed are:</em></p><ul>"
    #     for dl in deleted_dl_ids:
    #         detail = next((detail for detail in old_detail_planning_line if detail['id'] == dl), None)
    #         if detail:
    #             if dl in section_dl_ids:
    #                 new_sect = self.env['mrp.detail.planning.line'].browse(dl)
    #                 message += (
    #                     f"<li><p><b>{detail['name']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{new_sect.name}</span></b><em>(Large Section)</em></p></li>")
    #             elif dl in line_dl_ids:
    #                 new_line = self.env['mrp.detail.planning.line'].browse(dl)
    #                 message += f"<li><p><b>{detail['name']} <span style='font-size: 1.5em;'>&#8594;</span> <span style='color: #0182b6;'>{new_line.name}</span></b><em>(Large Line)</em></p></li>"
    #             else:
    #                 sect_dl = max([sect_id for sect_id in section_dl_ids if sect_id < dl], default=None)
    #                 line_dl = max([line_id for line_id in line_dl_ids if line_id < dl], default=None)
    #                 large_section = self.env['mrp.detail.planning.line'].browse(sect_dl)
    #                 large_line = self.env['mrp.detail.planning.line'].browse(line_dl)
    #                 message += f"<li><p><b>{detail['product_id']['name']}, large section {large_section.name}, large line {large_line.name}</b></p></li>"
    #     if message != "<p><em>Detailed planning lines removed are:</em></p><ul>":
    #         message += "</ul>"
    #         mrp_planning.message_post(body=message)
